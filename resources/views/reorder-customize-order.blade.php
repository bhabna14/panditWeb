{{-- resources/views/reorder-customize-order.blade.php --}}
...
@section('styles')
    ...
    <style>
        ...
        .card { border: 1px solid var(--border); border-radius: 12px; box-shadow: 0 4px 18px rgba(16,24,40,.06); background: var(--surface); }
...
@section('content')
    <form id="reorderForm"
          action="{{ route('reorderCustomizeOrder.store', ['id' => $original->id]) }}"
          method="post"
        ...
        <div class="mb-2">
            <span class="badge bg-secondary">Re-order for Request: {{ $original->request_id }}</span>
        </div>
        ...
        <div class="row g-3 mb-2">
            <div class="col-12">
                ...
                <select class="form-control select2" id="userid" name="userid_display" disabled>
                    @foreach ($user_details as $user)
                        <option value="{{ $user->userid }}" {{ $user->userid == $original->user_id ? 'selected' : '' }}>
                            {{ $user->userid }} - ({{ $user->mobile_number }})
                        </option>
                    @endforeach
                </select>
                <input type="hidden" name="userid" value="{{ $original->user_id }}">
                <div class="muted-hint mt-1">User is fixed to the original requester.</div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="your-address-list" id="addressContainer">
                    <div class="muted-hint">Loading addresses…</div>
                </div>
            </div>
        </div>
        ...
        <div id="flower-container">
            @php $items = $original->flowerRequestItems ?? collect(); @endphp
            ...
            @if($items->count())
                @foreach($items as $idx => $it)
                    <div class="row mb-3 input-wrapper">
                            ...
                            <select name="flower_name[]" class="form-control" required>
                                @foreach ($singleflowers as $singleflower)
                                    <option value="{{ $singleflower->name }}"
                                        {{ $singleflower->name == $it->flower_name ? 'selected' : '' }}>
                                        {{ $singleflower->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12 col-md-3">
                            ...
                            <input type="text" name="flower_quantity[]" required class="form-control"
                                   value="{{ $it->flower_quantity }}" placeholder="e.g. 1.5">
                        </div>
                        <div class="col-12 col-md-3">
                            ...
                            <select name="flower_unit[]" class="form-control" required>
                                @foreach ($Poojaunits as $Poojaunit)
                                    <option value="{{ $Poojaunit->unit_name }}"
                                        {{ $Poojaunit->unit_name == $it->flower_unit ? 'selected' : '' }}>
                                        {{ $Poojaunit->unit_name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                @endforeach
            @else
                {{-- Fallback: one empty row (shouldn’t normally happen) --}}
                <div class="row mb-3 input-wrapper">
                    <div class="col-12 col-md-3">
                        <label class="form-label">Flower <span class="text-danger">*</span></label>
                        <select name="flower_name[]" class="form-control" required>
        ...
        <div class="row mt-3">
            <div class="col-12 d-flex gap-2">
                <button type="submit" class="btn btn-primary" id="submitBtn">
                    <i class="fas fa-redo me-1"></i> Create Re-order
                </button>
...
@section('scripts')
    ...
    <script>
        $(function () {
            ...
            // Load addresses for original user and preselect original address
            const userId = $('input[name="userid"]').val();
            const originalAddressId = {{ $original->address_id }};
            ...
            function renderAddresses(addresses) {
                if (addresses && addresses.length) {
                    ...
                    addresses.forEach((address, index) => {
                        ...
                        const checked = Number(address.id) === Number(originalAddressId) ? 'checked' : '';
                        html += `
                              ...
                              <div class="card-body">
                                <div class="form-check">
                                  <input class="form-check-input" type="radio" name="address_id"
                                         id="address${address.id}" value="${address.id}" required ${checked}>
                    ...
                    container.innerHTML = '<div class="muted-hint">No addresses found for the user.</div>';
                }
            }

            fetch(`/admin/get-user-addresses/${userId}`)
                .then(r => r.json())
                .then(data => renderAddresses(data.addresses || []))
                .catch(() => {
                    container.innerHTML = '<div class="muted-hint">Failed to load addresses.</div>';
                    Swal.fire({ icon:'error', title:"Couldn't load addresses", text:'Please refresh and try again.' });
                });

            // Add/Remove flower rows
            $("#addFlower").on('click', function () {
                $("#flower-container").find('.row.mb-3').last().before(`
                    <div class="row mb-3 input-wrapper">
                        <div class="col-12 col-md-3">
            ...
            form.addEventListener('submit', async function (e) {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        window.location.href = "{{ route('flower.customize.request', ['filter' => 'all']) }}";
                    ...
                    submitBtn.innerHTML = '<i class="fas fa-redo me-1"></i> Create Re-order';
                }
            });
        });
    </script>
