...
@section('styles')
    ...
    <style>
        ...
        .benefit-card .card { transition: box-shadow .15s ease, transform .05s ease, border-color .15s ease; border: 1px solid #e6e6e6; }
        .benefit-card .card:hover { box-shadow: 0 6px 16px rgba(0,0,0,.07); }
        .benefit-card input[type="checkbox"]:checked + .card { border-color: #0d6efd; box-shadow: 0 0 0 .2rem rgba(13,110,253,.15); }
        .benefit-card .pair-num { font-weight: 600; }
        .benefit-toolbar .btn { margin-right: .5rem; }
    </style>
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('refer.saveOfferClaim') }}" method="POST" enctype="multipart/form-data" id="offer-claim-form">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div id="pairs-wrapper" class="d-none">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <div class="benefit-toolbar">
                                    <button type="button" class="btn btn-sm btn-outline-secondary" id="benefit-clear">Clear</button>
                                    <button type="button" class="btn btn-sm btn-outline-secondary" id="benefit-select-all">Select All</button>
                                </div>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $offer.on('change', function () {
                ...
                if (!len) {
                    $grid.append('<div class="col-12"><div class="alert alert-secondary mb-0">No benefit options for this offer.</div></div>');
                    return;
                }
            ...
            $(document).on('change', '.benefit-check', function() {
                ...
                if ($chk.is(':checked')) $icon.removeClass('d-none'); else $icon.addClass('d-none');
            });
