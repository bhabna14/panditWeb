...
@section('styles')
    ...
    <style>
        ...
        .metric-card {
            ...
            border-radius: 16px;
            transition: .2s;
            box-shadow: 0 6px 18px rgba(19, 33, 68, .04);
            position: relative;
            overflow: hidden;
        }
        ...
        .metric-accent::after {
            ...
            width: 120px;
            height: 120px;
            background: radial-gradient(rgba(124, 77, 255, .18), transparent 60%);
            border-radius: 50%;
        }
        ...
        .card-elevated {
            border-radius: 16px;
            box-shadow: 0 8px 20px rgba(19, 33, 68, .06);
            border: 1px solid #edf0f5
        }
        ...
        /* remove extra bottom space in card */
        .page-link .bi {
    ...
    <div class="container-fluid py-3">
        ...
        <div class="page-hero mb-3">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    <span class="soft-badge"><i class="bi bi-clock me-1"></i> Updated:
                        {{ now()->format('Y-m-d H:i') }}</span>
                </div>
            </div>
        </div>
                        ...
                        <div class="w-100">
                            ...
                            <div class="fw-semibold" style="line-height: 1.9;">
                                @forelse($platformBreakdown as $row)
                                    <span class="soft-badge me-1">{{ $row->platform ?? 'Unknown' }}:
                                        {{ $row->users }}</span>
                                @empty
        ...
        <form method="GET" class="filter-bar mb-3">
            <div class="row g-2 align-items-stretch">
                {{-- NEW: User selector (name/mobile) --}}
                <div class="col-md-4">
                    <select id="user_select" name="user_id" class="form-select"
                        data-placeholder="Find user by name or mobile">
                        @if (!empty($user_id) && $selectedUser)
                            {{-- Preselect currently chosen user so Select2 shows the label on load --}}
                            <option value="{{ $selectedUser->userid }}" selected>
                                {{ $selectedUser->mobile_number ?? '—' }} — {{ $selectedUser->name ?? 'Unnamed' }}
                            </option>
                        @endif
                    </select>
                    <div class="small text-muted mt-1">Filter by user (independent of device search).</div>
                </div>

                <div class="col-md-4 position-relative">
                    <i class="bi bi-search input-icon"></i>
                    <input type="text" name="search" class="form-control with-icon"
                        placeholder="Search device id, platform, model, version" value="{{ $search }}">
                </div>
                ...
                <div class="col-md-2">
                    <select class="form-select" name="platform">
                        ...
                        @foreach ($platforms as $p)
                            <option value="{{ $p }}" @selected($platform === $p)>{{ $p ?: 'Unknown' }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-2 position-relative">
                    ...
                    <input type="text" name="date_range" class="form-control with-icon"
                        placeholder="YYYY-MM-DD to YYYY-MM-DD" value="{{ $date_range }}">
                </div>
            </div>
            ...
            <div class="mt-2 d-flex align-items-center gap-3">
                ...
                @if ($search || $platform || $date_range || $user_id)
            ...
            <div class="col-lg-9">
                <div class="card card-elevated">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="small-muted">Showing {{ $devices->firstItem() }}–{{ $devices->lastItem() }} of
                            {{ $devices->total() }}</div>
                    </div>
                            ...
                            <div class="d-flex justify-content-between border-bottom py-2">
                                ...
                                <div class="text-end">
                                    <div class="mono small">
                                        {{ \Carbon\Carbon::parse($log->last_login_time)->format('H:i') }}</div>
                                    <div class="text-muted small">
                                        {{ \Carbon\Carbon::parse($log->last_login_time)->diffForHumans() }}</div>
                                </div>
                            </div>
                        @empty
                            <div class="empty-state">
