        ...
        <div class="table-responsive">
            <table class="table table-bordered align-middle table-hover">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="8" class="text-center py-4">No pending payments 🎉</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                <tbody>
                    @forelse($expiredSubs as $i => $row)
                        ...
                        <tr>
                            ...
                            <td>{{ $row->mobile_number }}</td>
                            <td>{{ $start->format('d M Y') }} — {{ $end->format('d M Y') }} <span class="text-muted small">({{ $durationDays }}d)</span></td>
                            <td>
                                {{ $row->product_category ?? '—' }}
                                @if ($row->product_name)
                                    <span class="text-muted small">({{ $row->product_name }})</span>
    ...
    $('#collectForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                modal.hide();
                Swal.fire({ icon: 'success', title: 'Done', text: res.message || 'Payment marked as paid.', timer: 1400, showConfirmButton: false });
