    ...
    <div class="rs-hero">
        <div>
            ...
            <p class="rs-sub">
                ...
                @else
                    If any rider salary is missing, system uses fallback: <b>₹{{ number_format($defaultMonthlySalary, 0) }}</b>.
                @endif
            </p>
        </div>
                ...
                <select name="rider_id" class="form-select rs-input">
                    ...
                    @foreach ($riders as $r)
                        <option value="{{ $r->rider_id }}" {{ $selectedRiderId == $r->rider_id ? 'selected' : '' }}>
                            {{ $r->rider_name }} ({{ $r->phone_number ?? 'N/A' }}) - Salary: ₹{{ number_format(($r->salary ?? $defaultMonthlySalary), 0) }}
                        </option>
                    @endforeach
                </select>
            </div>
                        ...
                        @foreach ($dayRows as $dr)
                            ...
                            <div class="rs-dayrow">
                                <div>
                                    ...
                                    <div class="rs-daymeta">In: {{ $dr['check_in'] ?? '-' }} | Out: {{ $dr['check_out'] ?? '-' }}</div>
                                </div>
                                <div class="text-end">
