...
@section('styles')
    ...
    <style>
        ...
        /* Card & hero */
        .nu-card{
            border:1px solid #e8ecf5; border-radius:16px; background:#fff;
            box-shadow:0 10px 28px rgba(17,24,39,.07)
        }
        .nu-hero{
            background:linear-gradient(135deg, #eef2ff 0%, #e0f7ff 100%);
            border:1px solid #e8ecf5; border-radius:16px
        }
        .nu-title{ font-weight:800; letter-spacing:.2px; color:var(--nu-slate) }

        /* Badges */
        .badge-status{ padding:.35rem .6rem; border-radius:999px; font-weight:700; font-size:.78rem }
        .badge-sent{ background:#e6fffa; color:#047857; border:1px solid #99f6e4 }
        .badge-partial{ background:#fff7ed; color:#b45309; border:1px solid #fed7aa }
        .badge-failed{ background:#fee2e2; color:#991b1b; border:1px solid #fecaca }
        .badge-queued{ background:#eef2ff; color:#1d4ed8; border:1px solid #c7d2fe }

        .form-hint{ font-size:.85rem; color:var(--nu-muted) }

        /* Image preview */
        .img-preview{
            max-height:100px; border:1px dashed #cbd5e1; border-radius:12px;
            padding:6px; background:#f8fafc
        }

        /* Select2 look */
        .select2-container--default .select2-selection--multiple{
            border:1px solid #e5e7eb; min-height:45px; padding:6px 8px; border-radius:.5rem
        }
        .select2-container--default .select2-selection--single{
            height:45px; border:1px solid #e5e7eb; border-radius:.5rem
        }
        ...
        .btn-primary{
            background:var(--nu-primary); border-color:var(--nu-primary);
            box-shadow:0 6px 12px rgba(37,99,235,.25)
        }
        .btn-primary:hover{ filter:brightness(.95) }
        .btn-outline-primary{ color:var(--nu-primary); border-color:var(--nu-primary) }
        .btn-outline-primary:hover{ background:var(--nu-primary-50) }

        /* Tiny tabs underline look */
        .tab-active{ border-bottom:3px solid var(--nu-primary); color:#1e293b !important }
    </style>
@endsection

@section('content')
    {{-- Expose prefill user id for JS --}}
    <script> window.__PREFILL_USER_ID__ = @json($prefillUserId ?? null); </script>
        ...
        <div class="nu-hero p-4 mb-4">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div>
                    ...
                    <div class="text-muted">Push a rich app notification to your users. Choose audience, attach an image, and preview.</div>
                </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-5">
                <div class="nu-card p-4 mb-4">
                    <form action="{{ route('admin.notification.send') }}" method="POST" enctype="multipart/form-data" id="fcmForm">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex gap-4 flex-wrap">
                                ...
                                <label class="form-check d-flex align-items-center gap-2">
                                    <input class="form-check-input" type="radio" name="audience" id="audUsers" value="users">
                        ...
                        <div class="mb-3 d-none" id="usersWrap">
                            ...
                            <select class="form-control" name="users[]" id="users" multiple data-placeholder="Search users…">
                                @foreach ($users as $u)
                                    <option value="{{ $u->userid }}">{{ $u->name }} — {{ $u->email ?? $u->mobile_number }}</option>
                                @endforeach
                            </select>
                            <div class="form-hint">Multi-select. Start typing a name, email, or number.</div>
                            @error('users') <div class="text-danger small">{{ $message }}</div> @enderror
                            @error('users.*') <div class="text-danger small">{{ $message }}</div> @enderror
                        </div>
                        ...
                        <div class="mb-3 d-none" id="platformWrap">
                            ...
                            <select class="form-select" name="platform[]" id="platform" multiple data-placeholder="Choose platform(s)">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($notification->image)
                                                <img src="{{ asset('storage/' . $notification->image) }}" width="40" class="rounded border">
                                            @else
                                                —
                                            @endif
                                        </td>
...
@section('scripts')
    ...
    <script>
        ...
        function toast(type, title, timer=1600){
            try {
                Swal.fire({toast:true, position:'top-end', showConfirmButton:false, timer, icon:type, title});
            } catch(e){}
        }
        function confirmDelete(e){
            e.preventDefault();
            Swal.fire({
                icon:'warning', title:'Delete this notification?', text:'This action cannot be undone.',
        ...
        function resendNotification(id){
            Swal.fire({
                icon:'question', title:'Resend this notification?', showCancelButton:true,
                confirmButtonText:'Resend', confirmButtonColor:'#2563eb'
            }).then(res => {
                if(res.isConfirmed){
        ...
        (function(){
            ...
            image.addEventListener('change', (e) => {
                ...
                if (!f) {
                    imgPreview.classList.add('d-none'); imgPreview.src = '';
                    return;
                }
                const url = URL.createObjectURL(f);
                imgPreview.src = url; imgPreview.classList.remove('d-none');
            });
