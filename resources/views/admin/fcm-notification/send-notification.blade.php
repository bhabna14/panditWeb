...
@section('styles')
    ...
    <style>
        ...
        .nu-hero { background: linear-gradient(135deg,#eef2ff 0%,#e0f7ff 100%); border: 1px solid #e8ecf5; border-radius: 14px }
        .nu-title { font-weight: 800; letter-spacing: .2px }
        ...
        .badge-queued { background:#eff6ff; color:#1d4ed8; border:1px solid #bfdbfe }
        ...
        .select2-container .select2-selection--multiple { min-height:42px; padding:6px 8px }
        .tab-active { border-bottom:3px solid #2563eb; color:#1e293b !important }
    </style>
@endsection
                ...
                <div class="nu-card p-4 mb-4">
                    <form action="{{ route('admin.notification.send') }}" method="POST" enctype="multipart/form-data" id="fcmForm">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-hint mt-1">Target everyone, a segment, or specific platforms (Android / iOS / Web).</div>
                        </div>
                                ...
                                @foreach ($notifications as $notification)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="text-muted small">{{ \Illuminate\Support\Str::limit($notification->description, 80) }}</div>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-nowrap">
                                            <form action="{{ route('admin.notifications.delete', $notification->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this notification?');">
                                                @csrf @method('DELETE')
                                                <button class="btn btn-sm btn-outline-danger">Delete</button>
                                            </form>
                                            <form id="resend-form-{{ $notification->id }}" action="{{ route('admin.notifications.resend', $notification->id) }}" method="POST" class="d-none">@csrf</form>
                                            <button class="btn btn-sm btn-outline-primary" onclick="resendNotification({{ $notification->id }})">Resend</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
@section('scripts')
    ...
    <script>
        // Select2
        const $usersSel = $('#users').select2({ placeholder: 'Search users…', width: '100%' });
        ...
        // Prefill single user (?user=USERID)
        (function prefillSingleUser() {
            ...
            try {
                Swal.fire({ toast:true, position:'top-end', timer:1400, showConfirmButton:false, icon:'info', title:'User preselected from Payment Collection' });
            } catch(e){}
        })();
