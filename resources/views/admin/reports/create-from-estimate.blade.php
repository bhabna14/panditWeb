...
@section('content')
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('admin.flowerEstimate', ['preset' => 'tomorrow']) }}" class="btn btn-outline-secondary">
                Back to Estimate
            </a>
        </div>
        ...
        <form method="POST" action="{{ route('admin.saveFlowerPickupAssignRider') }}" novalidate>
            ...
            @php
                ...
                $rows = $rowCountOld > 0
                    ? range(0, $rowCountOld - 1)
                    : range(0, max(count($prefillRows), 1) - 1);
            @endphp
                                        ...
                                        <select name="flower_id[]"
                                            ...
                                            @foreach ($flowers as $flower)
                                                <option value="{{ $flower->product_id }}"
                                                        @selected($flowerVal == $flower->product_id)>
                                                    {{ $flower->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="row_vendor_id[]"
                                            ...
                                            @foreach ($vendors as $v)
                                                <option value="{{ $v->vendor_id }}"
                                                        @selected($vendorVal == $v->vendor_id)>
                                                    {{ $v->vendor_name }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="mt-3 d-flex justify-content-between flex-wrap gap-2">
                        ...
                        <div class="totals-bar ms-auto">
                            ...
                            <small class="text-muted d-block mt-1">
                                Estimated uses Estimate Qty & Unit with live prices (constant);
                                Actual uses Actual Qty × Actual Price.
                            </small>
                        </div>
                    </div>
                </div>
