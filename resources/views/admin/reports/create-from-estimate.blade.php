    ...
    <form method="POST" action="{{ route('admin.saveFlowerPickupAssignRider') }}" novalidate>
        ...
        @php
            ...
            $rowCountOld = max(
                ...
                count($oldEstUnits), count($oldEstQtys), count($oldEstPrices),
        ...
        <div class="card mb-3">
            <div class="card-header bg-white d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>
                    <strong>Items</strong>
                    <div class="subhead">Enter <em>Estimate</em> (left) and <em>Actual</em> (right) per row</div>
                </div>
            ...
            <div class="card-body">
                    ...
                    <table class="table table-sm table-items align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th style="width:13%">Est. Unit</th>
                                <th style="width:11%">Est. Qty</th>
                                <th style="width:11%">Est. Price (₹)</th>
                            ...
                            @foreach ($rows as $i)
                                @php
                                    $default = array_merge(
                                        [
                                            'flower_id'    => null,
                                            // EST defaults from estimate prefill
                                            'est_unit_id'  => null,
                                            'est_quantity' => null,
                                            'est_price'    => null,

                                            // Actual defaults (blank)
                                            'unit_id'      => null,
                                            'quantity'     => null,
                                            'price'        => null,
                                    ...
                                    $estUnitVal = $oldEstUnits[$i]   ?? $default['est_unit_id'];
                                    $estQtyVal  = $oldEstQtys[$i]    ?? $default['est_quantity'];
                                    $estPriceVal= $oldEstPrices[$i]  ?? $default['est_price'];
                                ...
                                <tr data-row>
                                    ...
                                    {{-- Estimate Unit --}}
                                    <td>
                                        <select name="est_unit_id[]"
                                                class="form-control @error('est_unit_id.' . $i) is-invalid @enderror">
                                            <option value="" selected>Choose</option>
                                            @foreach ($units as $unit)
                                                <option value="{{ $unit->id }}" @selected($estUnitVal == $unit->id)>{{ $unit->unit_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('est_unit_id.' . $i)
                                            <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <td>
                                        <input type="number" name="est_price[]"
                                               class="form-control @error('est_price.' . $i) is-invalid @enderror"
                ...
                <template id="rowTemplate">
                    <tr data-row>
                        ...
                        <td>
                            <select name="est_unit_id[]" class="form-control">
                                <option value="" selected>Choose</option>
                                @foreach ($units as $unit)
                        ...
                        <td>
                            <input type="number" name="est_price[]" class="form-control" inputmode="decimal" min="0" step="0.01">
                        </td>
                ...
                <div class="mt-3 d-flex justify-content-between flex-wrap gap-2">
                    ...
                    <div class="totals-bar ms-auto">
                        ...
                        <small class="text-muted d-block mt-1">Totals are Σ(price × qty) for rows with both values provided.</small>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function attachRowHandlers(root) {
        ...
        (root || document).querySelectorAll('input[name="quantity[]"], input[name="price[]"], input[name="est_quantity[]"], input[name="est_price[]"]').forEach(inp => {
    ...
    function computeTotals() {
        estTotalEl.textContent = sumProduct('input[name="est_price[]"]', 'input[name="est_quantity[]"]').toFixed(2);
