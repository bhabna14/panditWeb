...
@section('content')
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('admin.flowerEstimate', ['preset' => 'tomorrow']) }}" class="btn btn-outline-secondary">Back to
                Estimate</a>
        </div>
                ...
                <div class="card-body">
                                    ...
                                    <tr data-row>
                                        ...
                                        <td>
                                            <select name="flower_id[]"
                                                class="form-control @error('flower_id.' . $i) is-invalid @enderror"
                                                required>
                                                <option value="" selected>Choose flower</option>
                                                @foreach ($flowers as $flower)
                                                    <option value="{{ $flower->product_id }}" @selected($flowerVal == $flower->product_id)>
                                                        {{ $flower->name }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <td>
                                            ...
                                            <select class="form-control" data-est-unit-display disabled>
                                                ...
                                                @foreach ($units as $unit)
                                                    <option value="{{ $unit->id }}" @selected($unitVal == $unit->id)>
                                        ...
                                        <td>
                                            <select name="row_vendor_id[]"
                                                ...
                                                @foreach ($vendors as $v)
                                                    <option value="{{ $v->vendor_id }}" @selected($vendorVal == $v->vendor_id)>
                                        ...
                                        <td>
                                            <select name="row_rider_id[]"
                                                ...
                                                @foreach ($riders as $r)
                                                    <option value="{{ $r->rider_id }}" @selected($riderVal == $r->rider_id)>
                                                        {{ $r->rider_name }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <td class="text-end">
                                            <button type="button"
                                                class="btn btn-outline-danger btn-sm remove-row-btn">Remove</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                    ...
                    <template id="rowTemplate">
                        <tr data-row>
                            ...
                            <td>
                                <input type="number" name="est_quantity[]" class="form-control" inputmode="decimal"
                                    min="0.01" step="0.01">
                            </td>
                            ...
                            <td>
                                <input type="number" name="quantity[]" class="form-control" inputmode="decimal"
                                    min="0.01" step="0.01">
                            </td>
                            <td>
                    ...
                    <div class="mt-3 d-flex justify-content-between flex-wrap gap-2">
                        ...
                        <div class="totals-bar ms-auto">
                            ...
                            <small class="text-muted d-block mt-1">Estimated uses Est. Qty × price-per Actual Unit (from
                                live prices).</small>
                        </div>
                    </div>
                </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // These may not exist in your current markup; guard all uses.
            const applyOneRiderCb = document.getElementById('applyOneRider') || null;
            const bulkRiderSel = document.getElementById('bulkRider') || null;
            ...
            function computeTotals() {
                ...
                rowsBody.querySelectorAll('tr[data-row]').forEach(tr => {
                    ...
                    const estQty = parseFloat(tr.querySelector('input[name="est_quantity[]"]')?.value ||
                        '0') || 0;
            ...
            // --------- Optional bulk rider helpers (no-op if controls missing) --------
            function syncRowRider(sel) {
                if (!bulkRiderSel) return;
            ...
            function syncAllRiders() {
                ...
                rowsBody.querySelectorAll('[data-row-rider]').forEach(sel => {
                    sel.value = val;
                    sel.disabled = true;
                });
            }
            ...
            function unlockAllRiders() {
                rowsBody.querySelectorAll('[data-row-rider]').forEach(sel => sel.disabled = false);
            }

            // ------------------------ Event delegation -------------------------------
            // Add Row
            addRowBtn.addEventListener('click', function() {
                const clone = document.importNode(rowTpl.content, true);
                rowsBody.appendChild(clone);

                // Get the row we just appended (last element child of tbody)
                const tr = rowsBody.querySelector('tr[data-row]:last-of-type');

                // If bulk rider mode is enabled, prefill and lock the new row rider
                if (applyOneRiderCb && bulkRiderSel && applyOneRiderCb.checked) {
                ...
                // Ensure Est. Unit shows for the new row
                syncEstimateUnit(tr);
                computeTotals();
            });
            ...
            // Inputs/Selects that affect totals or mirroring (delegated)
            ...
            // Bulk rider toggles (only if controls exist)
            if (applyOneRiderCb && bulkRiderSel) {
            ...
            // ----------- Initial hydration -----------
            // Ensure Est. Unit shows on existing rows and totals are computed
            rowsBody.querySelectorAll('tr[data-row]').forEach(tr => syncEstimateUnit(tr));
            computeTotals();

            // Rehydrate optional bulk-rider state without throwing when controls are absent
            if (applyOneRiderCb && bulkRiderSel && applyOneRiderCb.checked) {
                bulkRiderSel.disabled = false;
                syncAllRiders();
            }
