                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" id="delivery_date" name="delivery_date"
                                ...
                                value="{{ old('delivery_date', $prefillDate) }}" min="{{ now()->toDateString() }}"
                                required>
                            @error('delivery_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-sm table-items align-middle">
                            ...
                            <tbody id="rowsBody">
                                @foreach ($rows as $i)
                                    @php
                                        // ✅ merge with full defaults so all keys exist (prevents "Undefined array key 'price'")
                                        $default = array_merge(
                                            [
                                                'flower_id' => null,
                                                'unit_id' => null,
                    ...
                    <template id="rowTemplate">
                        <tr data-row>
                            ...
                            <td><input type="number" name="quantity[]" class="form-control" inputmode="decimal"
                                    min="0.01" step="0.01" required></td>
                            <td><input type="number" name="price[]" class="form-control" inputmode="decimal"
                                    min="0" step="0.01"></td>
                            <td class="text-end"><button type="button"
                                    class="btn btn-outline-danger btn-sm remove-row-btn">Remove</button></td>
                        </tr>
                    </template>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function attachRowHandlers(root) {
                ...
                (root || document).querySelectorAll('input[name="quantity[]"], input[name="price[]"]').forEach(
                    inp => {
