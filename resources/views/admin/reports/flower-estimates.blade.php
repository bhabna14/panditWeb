        ...
        <div class="container py-4">
            ...
            <div class="card border-0 shadow-sm mt-3">
                <div class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <small class="text-muted">From {{ \Carbon\Carbon::parse($start)->toFormattedDateString() }} to
                        {{ \Carbon\Carbon::parse($end)->toFormattedDateString() }}</small>
                </div>
                <div class="card-body">
                    <div class="row g-3">
            ...
            {{-- Your existing day/month accordions go here --}}

        </div>
    </div>

    {{-- ASSIGN PICKUP MODAL --}}
    <div class="modal fade" id="assignPickupModal" tabindex="-1" aria-labelledby="assignPickupLabel"
        aria-hidden="true">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" id="pickup_date" name="pickup_date" class="form-control"
                                            value="{{ $tomorrowDate ?? now()->addDay()->toDateString() }}"
                        ...
                        <div class="nu-card section-card mb-3">
                            <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-12">
                                ...
                                <div class="d-flex align-items-center gap-12">
                                    <span class="badge bg-light text-dark row-badge">Rows: <span
                                            id="rowCount">0</span></span>
                                        ...
                                        <div class="row g-3 align-items-end">
                                            <div class="col-md-4">
                                                ...
                                                <select name="flower_id[]" class="form-control" required>
                                                    ...
                                                    @foreach ($flowers ?? [] as $flower)
                                                        <option value="{{ $flower->product_id }}">{{ $flower->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="number" name="quantity[]" class="form-control"
                                                    placeholder="e.g. 10" inputmode="decimal" min="0.01"
                                                    step="0.01" required>
                                            </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function wireRow(rowRoot) {
                const removeBtn = rowRoot.querySelector('.remove-row-btn');
                if (removeBtn) {
            ...
            addRowBtn?.addEventListener('click', appendEmptyRow);
            ...
            function prefillFromTomorrow() {
                ...
                tomorrowTotals.forEach(item => {
                    ...
                    // Flower by exact name
                    const rawName = (item.item_name || '').trim();
                    const flowerId = flowerNameToId[rawName] ?? '';
                    if (flowerId) flowerSel.value = String(flowerId);

                    // Unit mapping
                    let dispUnit = String(item.total_unit_disp || '').trim().toLowerCase();
                    if (alias[dispUnit]) dispUnit = alias[dispUnit];
                    const unitId = unitSymbolToId[dispUnit] ?? '';
                    if (unitId) unitSel.value = String(unitId);

                    // Quantity (already display-scaled)
                    const qty = (item.total_qty_disp != null && item.total_qty_disp !== '') ? item
                        .total_qty_disp : 0;
                    qtyInput.value = qty;
