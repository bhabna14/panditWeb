...
@section('styles')
    <style>
        ...
        .chip.badge { background: #eefbf0; border-color: #d9f3de; }
        ...
        .note { font-size: .82rem; color: #5f6b7a; }
    </style>
@endsection
    ...
    <div class="card card-soft mb-4">
        ...
        <div class="card-body">
            @if (empty($tomorrowEstimate['lines']))
                <div class="muted">No deliveries for tomorrow.</div>
            @else
                    ...
                    <table class="table table-sm table-bordered align-middle">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th class="text-end amount" colspan="1">{{ number_format($tomorrowEstimate['total_qty'], 2) }}</th>
                                <th class="text-end amount" colspan="2"><strong>Total ₹ {{ number_format($tomorrowEstimate['total_cost'], 2) }}</strong></th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
    ...
    <div class="card card-soft mb-4">
        ...
        <div class="card-body">
            @if (empty($dayEstimate['lines']))
                <div class="muted">No deliveries on this day.</div>
            @else
                <div class="table-responsive">
                    <table class="table table-sm table-bordered align-middle">
                        <thead>
                        ...
                        <tfoot>
                            <tr>
                                <th colspan="2" class="text-end">Totals</th>
                                <th class="text-end amount" colspan="1">{{ number_format($dayEstimate['total_qty'], 2) }}</th>
                                <th class="text-end amount" colspan="2"><strong>Total ₹ {{ number_format($dayEstimate['total_cost'], 2) }}</strong></th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        <div class="card-body vstack">
            <div class="table-responsive">
                <table class="table table-sm table-bordered align-middle">
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="2" class="text-end">Month Totals</th>
                            <th class="text-end amount" colspan="1">{{ number_format($sumQty, 2) }}</th>
                            <th class="text-end amount" colspan="2"><strong>Total ₹ {{ number_format($monthEstimate['total_cost'], 2) }}</strong></th>
                        </tr>
                    </tfoot>
                </table>
            </div>
