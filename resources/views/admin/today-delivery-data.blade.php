                    ...
                    <tbody>
                        @foreach ($deliveries as $index => $delivery)
                            @php
                                ...
                                $address = $order?->address ?? null; // <-- fixed here
                            ...
                            <tr>
                                ...
                                <td>₹{{ $packagePricePerDay }}</td>
                                <td>{{ $delivery->created_at->format('h:i A') }}</td>
                                <td>{{ $delivery->rider->rider_name ?? '—' }}</td>
                                <td>
                                    @if ($user && $address)
                                        <button class="btn btn-sm btn-view-address text-white" data-bs-toggle="modal"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="modalLabel{{ $index }}">Customer
                                                            Address Details</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        <ul class="list-group">
                                                            <li class="list-group-item"><strong>Apartment:</strong>
                                                                {{ $address->apartment_name ?? '—' }}</li>
                                                            <li class="list-group-item"><strong>Flat/Plot:</strong>
                                                                {{ $address->apartment_flat_plot ?? '—' }}</li>
