...
@section('styles')
    ...
    <style>
        ...
        .img-preview { max-height: 90px; display: inline-block; margin-top: 6px; border-radius: 8px; }
...
@section('content')
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="categorySmartForm">
        @csrf
        @method('PUT')
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <div class="form-text-muted mt-1">Switching category will show/hide relevant fields; ensure you review before saving.</div>
            </div>
            ...
            <div class="col-md-4 mb-3 controlled" data-block="core">
                ...
                <input type="text" name="name" class="form-control" id="name" value="{{ old('name', $product->name) }}" placeholder="Enter product name" required>
            </div>

            <div class="col-md-4 mb-3 controlled" data-block="core">
                <label for="odia_name" class="form-label"><span id="label-odia-name">Product Name (Odia)</span></label>
                <input type="text" name="odia_name" class="form-control" id="odia_name" value="{{ old('odia_name', $product->odia_name) }}" placeholder="Enter product name in Odia">
            </div>

            <div class="col-md-4 mb-3 controlled" data-block="core">
                <label for="mrp" class="form-label"><span id="label-mrp">MRP (Rs.)</span></label>
                <input type="number" name="mrp" class="form-control" id="mrp" min="0" step="0.01" value="{{ old('mrp', $product->mrp) }}" placeholder="Enter MRP" required>
            </div>

            <div class="col-md-4 mb-3 controlled" data-block="core">
                <label for="price" class="form-label"><span id="label-price">Sale Price (Rs.)</span></label>
                <input type="number" name="price" class="form-control" id="price" min="0" step="0.01" value="{{ old('price', $product->price) }}" placeholder="Enter sale price" required>
            </div>

            <div class="col-md-4 mb-3 controlled" data-block="core">
                <label for="discount" class="form-label"><span id="label-discount">Discount (%)</span></label>
                <input type="number" name="discount" class="form-control" id="discount" min="0" max="100" step="0.01" value="{{ old('discount', $product->discount) }}" placeholder="Enter discount percentage">
            </div>
            ...
            <div class="col-md-4 mb-3 controlled" id="stockGroup" data-block="stock">
                ...
                <input type="number" name="stock" class="form-control" id="stock" min="0" value="{{ old('stock', $product->stock) }}" placeholder="Enter stock quantity">
            </div>
            ...
            <div class="col-md-4 mb-3 controlled" data-block="core">
                ...
                @if ($product->product_image)
                    <div class="mt-2">
                        <img id="imagePreview" class="img-preview" src="{{ $product->product_image }}" alt="Current image">
            ...
            <div class="col-md-4 mb-3 controlled" id="malaProvidedField" data-block="flower">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="mala_provided" id="malaYes" value="yes" {{ old('mala_provided', $product->mala_provided === null ? null : ($product->mala_provided ? 'yes' : 'no')) === 'yes' ? 'checked' : '' }}>
                    <label class="form-check-label" for="malaYes">Yes</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="mala_provided" id="malaNo" value="no" {{ old('mala_provided', $product->mala_provided === null ? null : ($product->mala_provided ? 'yes' : 'no')) === 'no' ? 'checked' : '' }}>
            ...
            <div class="col-md-4 mb-3 controlled" id="flowerAvailabilityField" data-block="flower">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="flower_available" id="flowerActive" value="yes" {{ old('flower_available', $product->is_flower_available === null ? null : ($product->is_flower_available ? 'yes' : 'no')) === 'yes' ? 'checked' : '' }}>
                    <label class="form-check-label" for="flowerActive">Active</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="flower_available" id="flowerInactive" value="no" {{ old('flower_available', $product->is_flower_available === null ? null : ($product->is_flower_available ? 'yes' : 'no')) === 'no' ? 'checked' : '' }}>
            ...
            <div class="col-md-4 mb-3 controlled" id="flowerFromField" data-block="flowerDates">
                ...
                <input type="date" name="available_from" id="available_from" class="form-control" value="{{ old('available_from', $product->available_from ? \Carbon\Carbon::parse($product->available_from)->format('Y-m-d') : null) }}">
            </div>
            ...
            <div class="col-md-4 mb-3 controlled" id="flowerToField" data-block="flowerDates">
                ...
                <input type="date" name="available_to" id="available_to" class="form-control" value="{{ old('available_to', $product->available_to ? \Carbon\Carbon::parse($product->available_to)->format('Y-m-d') : null) }}">
            </div>
            ...
            <div class="col-md-4 mb-3 controlled" id="poojafields" data-block="package">
                ...
                <select class="form-control select2" id="pooja_id" name="pooja_id">
                    ...
                    @foreach ($pooja_list as $pooja)
                        <option value="{{ $pooja->id }}" {{ (string) old('pooja_id', $product->pooja_id) === (string) $pooja->id ? 'selected' : '' }}>{{ $pooja->pooja_name }}</option>
                    @endforeach
                </select>
            </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-control select2 item-select" name="item_id[]" required>
                                        ...
                                        @foreach ($flowerlist as $flower)
                                            <option value="{{ $flower->id }}" {{ (string) old('item_id.' . $idx, $selectedItem) === (string) $flower->id ? 'selected' : '' }}>{{ $flower->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Qty</label>
                                    <input type="number" class="form-control" name="quantity[]" min="0" step="any" value="{{ old('quantity.' . $idx, $packageItem->quantity) }}" placeholder="0" required>
                                </div>
                                <div class="col-md-3">
                                    ...
                                    <select class="form-control select2 unit-select" name="unit_id[]" required>
                                        ...
                                        @foreach ($pooja_units as $u)
                                            <option value="{{ $u->id }}" {{ (string) old('unit_id.' . $idx, $selectedUnit) === (string) $u->id ? 'selected' : '' }}>{{ $u->unit_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">Item Price (Rs.)</label>
                                    <input type="number" class="form-control" name="item_price[]" min="0" step="0.01" value="{{ old('item_price.' . $idx, $packageItem->price) }}" placeholder="0.00" required>
                                </div>
                            </div>
                        @endforeach
                    @else
            ...
            <div class="col-md-4 mb-3 controlled" id="durationGroup" data-block="subscription">
                ...
                <select name="duration" id="duration" class="form-control select2">
                    ...
                    <option value="1" {{ (string) old('duration', $product->duration) === '1' ? 'selected' : '' }}>1 Month</option>
                    <option value="3" {{ (string) old('duration', $product->duration) === '3' ? 'selected' : '' }}>3 Months</option>
                    <option value="6" {{ (string) old('duration', $product->duration) === '6' ? 'selected' : '' }}>6 Months</option>
                </select>
            </div>
            ...
            <div id="subscriptionDayFields" class="col-md-12 mb-3 controlled" data-block="subscription">
                ...
                <div class="row mb-3 align-items-end">
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" name="per_day_price" min="0" step="0.01" value="{{ old('per_day_price', $product->per_day_price) }}" placeholder="0.00">
                    </div>
                </div>
            </div>
                            ...
                            <div class="row mb-3 subscription-item-row align-items-end">
                                <div class="col-md-4">
                                    ...
                                    <select class="form-control select2 item-select" name="sub_item_id[]" required>
                                        <option value="">Select Puja List</option>
                                        @foreach ($flowerlist as $flower)
                                            <option value="{{ $flower->id }}" {{ (string) old('sub_item_id.' . $idx, $selectedItem) === (string) $flower->id ? 'selected' : '' }}>{{ $flower->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Qty</label>
                                    <input type="number" class="form-control" name="sub_quantity[]" min="0" step="any" value="{{ old('sub_quantity.' . $idx, $sItem->quantity) }}" placeholder="0" required>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">Unit</label>
                                    <select class="form-control select2 unit-select" name="sub_unit_id[]" required>
                                        <option value="">Select Unit</option>
                                        @foreach ($pooja_units as $u)
                                            <option value="{{ $u->id }}" {{ (string) old('sub_unit_id.' . $idx, $selectedUnit) === (string) $u->id ? 'selected' : '' }}>{{ $u->unit_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">Item Price (Rs.)</label>
                                    <input type="number" class="form-control" name="sub_item_price[]" min="0" step="0.01" value="{{ old('sub_item_price.' . $idx, $sItem->price) }}" placeholder="0.00" required>
                                </div>
                            </div>
                        @endforeach
                    @else
            ...
            <div class="col-md-4 mb-3 controlled" data-block="core">
                ...
                <div id="benefitFields">
                    @php
                        $benefits = collect(explode('#', (string) $product->benefits))->map(fn($b) => trim($b))->filter()->values();
                        if ($benefits->isEmpty()) { $benefits = collect(['']); }
                    @endphp

                    @foreach ($benefits as $i => $b)
                        <div class="input-group mb-2 benefit-row">
                            <input type="text" name="benefit[]" class="form-control" value="{{ old('benefit.' . $i, $b) }}" placeholder="Enter benefit" {{ $loop->first ? 'required' : '' }}>
                            <button type="button" class="btn btn-success add-benefit" title="Add Benefit">
                                <i class="fa fa-plus"></i> Add
                            </button>
                            <button type="button" class="btn btn-danger remove-benefit" title="Remove Benefit" style="display: {{ $benefits->count() > 1 ? 'inline-block' : 'none' }};">
            ...
            <div class="col-md-12 mb-3 controlled" data-block="core">
                ...
                <textarea name="description" class="form-control" id="description" rows="3" placeholder="Enter description" required>{{ old('description', $product->description) }}</textarea>
            </div>
    ...
    <script>
        ...
        (function () {
            ...
            const VISIBILITY = {
                ...
                default:         { core:true, stock:true,  subscription:false, flower:false, flowerDates:false, package:false }
            };
            ...
            function updateFlowerDatesRequired() {
                ...
                [availableFrom, availableTo].forEach(el => {
                    ...
                    el.disabled = !isFlower; // only disable when not flower; when flower but not active, keep enabled but not required
                });
            }
