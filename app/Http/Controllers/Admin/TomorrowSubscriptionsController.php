   ...
   public function index(Request $request)
{
    ...
    /**
     * Return true if we can construct a non-empty address for a subscription.
     */
    $hasDeliverableAddress = function ($s) {
        $user  = $s->users;
        $order = $s->order;

        // Try order shipping first
        if ($order) {
            foreach ([
        ...
        // Fallback to user's default address details
        if ($user && $user->addressDetails) {
            $ad = $user->addressDetails;
            foreach ([
                'apartment_flat_plot','apartment_name','landmark','area','city','state','pincode'
            ] as $f) {
                if (isset($ad->$f) && filled($ad->$f)) {
                    return true;
                }
            }
        }
    ...
    /**
     * 1) Tomorrow Delivery — UPDATED:
     *    - start_date MUST be exactly tomorrow
     *    - must have at least one paid flower_payment
     *    - must have a deliverable address
     */
    $activeTomorrow = Subscription::with($withSubs)
        ...
        ->whereDate('start_date', '=', $tomorrow->toDateString()) // exact tomorrow
        ->whereHas('flowerPayments', function ($q) {
            $q->where('payment_status', 'paid'); // must be paid
        })
        ->get()
        // exclude if paused on the target day
        ...
        // exclude rows with no deliverable address
        ->filter($hasDeliverableAddress)
        // keep your original "shouldHide"
        ->reject($shouldHide)
        ->values();

    // 2) Starting Tomorrow — NEW USERS ONLY (kept same as before)
    ...
    // 3) Pausing from Tomorrow (unchanged)
    ...
    // 4) Tomorrow Customize Orders (unchanged)
    $customizeTomorrow = FlowerRequest::with($withRequests)
        ->whereDate('date', '=', $tomorrow->toDateString())
        ->get();

    // 5) Pause → Active from Tomorrow (pause_end_date == today) (unchanged)
    ...
    // 6) Expired Today — Users (unchanged)
    ...
    // Totals card (computed from updated activeTomorrow)
