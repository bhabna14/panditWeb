    ...
    public function index(Request $request)
{
    ...
    /**
     * 7) CANONICAL "TOMORROW TOTALS" (MATCHES tomorrowFlower)
     * -----------------------------------------------------
     * We REUSE the exact same logic used in tomorrowFlower():
     *   - FlowerDetails live price index
     *   - fetchActiveSubsEffectiveOn()
     *   - buildEstimateForSubsOnDate()
     *   - buildRequestsProductBlock()
     *   - recomputeTotalsByItemFromProducts()
     *
     * This ensures the totals grid on "Tomorrow Subscriptions" page
     * matches the "Tomorrow Flower" page 1:1.
     */
    ...
    // Use the same effective-subs logic as tomorrowFlower
    $tomorrowSubsForEstimate = $this->fetchActiveSubsEffectiveOn($tomorrow);

    // Build estimate for tomorrow subs
    $tomorrowEstimate = $this->buildEstimateForSubsOnDate($tomorrowSubsForEstimate, $tomorrow, $fdIndex);

    // Merge Flower Requests for tomorrow (so requests are included in totals as well)
    ...
    // This is what the totals grid uses now (same as tomorrowFlower)
    ...
    $data = [
        ...
        // NEW: canonical totals (now match tomorrowFlower)
        'tTotals'              => $tTotals,
    ];
