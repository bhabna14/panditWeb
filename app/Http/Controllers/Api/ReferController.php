...
class ReferController extends Controller
{
    ...
    public function saveOfferClaim(Request $request)
    {
        try {
            ...
            // 6) Always INSERT a new claim row + mark referrals as claimed in one transaction
            $claim = DB::transaction(function () use ($userId, $offerId, $pairPayload, $dt) {
                // NOTE: Do NOT look up by (user_id, offer_id). Always create a new row.
                $row = new ReferOfferClaim();
                $row->user_id        = $userId;
                ...
                // Mark all referral rows for this user as claimed = 'yes' if not already
                FLowerReferal::where('user_id', $userId)
                    ->where(function ($q) {
                        $q->whereNull('claimed_status')
                        ->orWhere('claimed_status', '!=', 'yes');
                    })
                    ->update(['claimed_status' => 'yes']);
