...
class VendorOtpController extends Controller
{
    ...
    public function loginPassword(Request $request)
    {
        ...
        if (self::looksHashed($stored)) {
            ...
            if ($valid) { 
                $vendor->password = $inputPassword; // model mutator will hash it
                $vendor->save(); 
            }
        }
   ...
   public function sendOtp(Request $request)
{
    ...
    // Normalize phone and build all possible candidates
    $candidates = $this->phoneCandidates($validated['phone']);

    // Find vendor by phone number variants
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if (empty($validated['phone']) && empty($validated['vendor_id'])) {
            return response()->json(['success' => false, 'message' => 'Provide either phone or vendor_id.'], 422);
        }
